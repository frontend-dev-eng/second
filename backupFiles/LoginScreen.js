...
import { Typography, Box, CircularProgress, useTheme, Link, Tabs, Tab, FormControlLabel, Checkbox, Divider } from '@mui/material';
...
const LoginScreen = () => {
    ...
    return (
        <div
            className={`login-page ${t("backgroundImage") ? "img" : ""}`}
            style={{ backgroundImage: t("backgroundImage") ? `url(${t("backgroundImage")})`: "",}}
        >
        <div className="main-container">
            <div className='main'>
                ...
                <h2 className="sub-heading"> Assign Your Locker</h2>
                <p className="paragraph">Please provide your phone number or email address for booking confirmation and access. </p>
                <Box className="input-tab-durolt-container">
                    <Tabs 
                        className="tab-bar"
                        ...
                        >
                    {isMobileOtpEnabled && (<Tab className="tab-bar-label"  label={t("phoneNumberLabel")}/>)}
                    {isEmailOtpEnabled && (<Tab className="tab-bar-label" label="Email"/>)}
                    </Tabs>
                </Box>
                {(assignmentSettings?.usecase_settings?.verify_with_mobile_otp_pwa?.value ||
                    assignmentSettings?.usecase_settings?.verify_with_email_otp_pwa?.value) && (
                <div className="form-controller-container">
                    ...
                    <div className="login-button-container">
                        <ButtonComponent 
                            ...
                            title={ isLoading ? (<CircularProgress size={24} />) 
                            :  t("loginBtnText")}
                            disabled={customer === "Durolt" && disabled}
                        />
                    </div>
                </div>
                )}
            
                {/* this is for extra way of login  */}
                {(hasExtraAuthMethods(assignmentSettings?.usecase_settings) ||
                (isLoginWithGoogle && isLoginWithFacebook)) && 
                <div className={customer === "HiveBoard" ? "hive-divider" : "divider"}>
                    <span className="line" />
                    <span className="text">{t('orText')}</span>
                    <span className="line" />
                </div>
                ...
                {isLoginWithGoogle && (
                    <ButtonComponent
                        buttonId="login-with-google-button"
                        handleClick={() => {}}
                        title={isGoogleLoginLoading ? (<CircularProgress size={24} />) : (t("loginWithGoogleLabel"))}
                        disabled={null}
                        customClass="blurred"
                ...
                {isLoginWithFacebook && (
                    <ButtonComponent
                        buttonId="login-with-facebook-button"
                        handleClick={() => {}}
                        title={isFacebooLoginkLoading ? (<CircularProgress size={24} />) : (t("loginWithFacebookLabel"))}
                        disabled={null}
                        customClass="blurred"
